...
temp_preds_family=temp_preds_family.reshape(len(temp_preds_family),1)
loss_family=hamming_loss(Y_family,temp_preds_family)
print('\tHamming Loss:',loss_family)
...
temp_preds_genus=temp_preds_genus.reshape(len(temp_preds_genus),1)
loss_genus=hamming_loss(Y_genus,temp_preds_genus)
print('\tHamming Loss:',loss_genus)
...
temp_preds_species=temp_preds_species.reshape(len(temp_preds_species),1)
loss_species=hamming_loss(Y_species,temp_preds_species)
print('\tHamming Loss:',loss_species)

# majority triplets per cluster

print("\nCluster-0 Majority Triplet: ",majority_family[c_0_family_label]," ",majority_genus[c_0_genus_label]," ",majority_species[c_0_species_label])
print("Cluster-1 Majority Triplet: ",majority_family[c_1_family_label]," ",majority_genus[c_1_genus_label]," ",majority_species[c_1_species_label])
# calculating losses

arr=[]
arr.append(loss_family)
arr.append(loss_genus)
arr.append(loss_species)
arr=np.array(arr)
...
misclassified=0
for i in range(0,len(labels)):
    if((temp_preds_family[i]==Y_family[i]) and (temp_preds_genus[i]==Y_genus[i]) and (temp_preds_species[i]==Y_species[i])):
        correct_classification+=1
    else:
        misclassified+=1

net_zero_one=misclassified/len(labels)

print("Average Zero-one Loss:",net_zero_one)
